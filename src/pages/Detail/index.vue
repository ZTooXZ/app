      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="changeShuNum" />
...
export default {
  ...
  computed: {
    ...
    // 给放大镜 子组件的数据  如果服务器数据没有返回，当前对象会是 undefined ，空对象.属性 会报错
    skuImageList() {
  ...
  methods: {
    ...
    changeActive(spuSaleAttrValue, arr) {
      // 排他 遍历所有的元素 让 isChecked=0 让所有元素都没有高亮了
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      // 点击的售卖属性
    ...
    // 表单元素修改商品个数
    changeShuNum(event) {
      // 让用户输入 的 文本 * 1 ---当不是数字的文本*1 时 会变成NaN
    ...
    async addShopCart() {
      /* this.$store.dispatch('addOrUpdateShopCart') 
        其实就是调用 vuex仓库中的addOrUpdateShopCart()函数
        也就是一个Promise函数 */
      try {
        /*  vuex 接口返回成功--  添加购物车成功  并路由跳转*/
        await this.$store.dispatch("addOrUpdateShopCart", {
          skuId: this.$route.params.skuId,
          skuNum: this.skuNum,
        });
        // 路由跳转 ,还需要传递 产品的信息给下级路由组件
        this.$router.push({
          name: "addcartsuccess",
          query: { skuNum: this.skuNum },
        });
        /* 产品信息的数据，通过会话存储（不持久、会话结束数据再消失）
        本地存储|会话存储 一般存储的都是字符串 */
